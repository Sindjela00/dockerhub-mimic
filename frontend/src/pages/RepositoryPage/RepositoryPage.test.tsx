...
export default function RepositoryDetailPage() {
  ...
  // Only show Teams tab when the repo belongs to an org
  const orgName = repo.organization?.name ?? null;

  // Build tab list — inject "teams" only when there's an org
  const visibleTabs = orgName ? TABS : TABS.filter((t) => t.value !== "teams");
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="flex flex-col gap-5">
        ...
        {activeTab === "teams" && orgName && <TeamsTab orgName={orgName} />}
      </div>
